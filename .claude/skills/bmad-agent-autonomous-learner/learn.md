...
- **category**: 知识分类 (产品/运营/商业化/数据/技术/质量/商务/行业) - **可选**，如未指定则自动从内容中识别
- **source**: 学习来源 (URL/文件路径/文本内容)
- **source_type**: 来源类型 (url/file/text) - 如未指定，根据 source 内容自动判断

**知识分类体系**：读取 `docs/knowledge/_taxonomy.yaml` 获取顶级分类和子目录映射。
...
3. **存储到对应分类**
   ...
   **子目录映射**：从 `docs/knowledge/_taxonomy.yaml` 读取顶级分类对应的子目录。
...
5. **知识沉淀**
   - 提取关键洞察（1-3条）
   - 追加到 `docs/knowledge/{category}/知识沉淀.md`
   - 更新来源追踪表
   - 更新 `docs/knowledge/知识沉淀总览.md`
